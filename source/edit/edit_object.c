...
bool	edit_object(int key_input, t_vars *vars)
{
	...
	ret = false;
	if (vars->edit.num == -1 && ((key_input >= 18 && key_input <= 23) || (key_input >= 25 && key_input <= 29 && key_input != 27)))
	{
		...
		ret = edit_sphere(key_input, vars->descr.c.o, vars->descr.sp[vars->edit.num]);
	else if (vars->edit.obj == PLANE)
		ret = edit_plane(key_input, vars->descr.c.o, vars->descr.pl[vars->edit.num]);
	else if (vars->edit.obj == CYLINDER)
		ret = edit_cylinder(key_input, vars->descr.c.o, vars->descr.cy[vars->edit.num]);
	return (ret);
}
...
static bool	edit_plane(int key_input, t_vec3 cam, t_plane *pl)
{
	...
	if (key_input == KEY_I || key_input == KEY_J || key_input == KEY_K || key_input == KEY_L || key_input == KEY_U || key_input == KEY_O)
...
static bool	edit_cylinder(int key_input, t_vec3 cam, t_cylinder *cy)
{
	...
	if (key_input == KEY_I || key_input == KEY_J || key_input == KEY_K || key_input == KEY_L || key_input == KEY_U || key_input == KEY_P)
